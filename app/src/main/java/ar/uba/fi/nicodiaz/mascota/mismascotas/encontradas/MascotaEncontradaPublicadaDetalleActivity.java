...
public class MascotaEncontradaPublicadaDetalleActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        WaitForInternetCallback callback = new WaitForInternetCallback(this) {
            public void onConnectionSuccess() {
                setContentView(R.layout.activity_mascota_encontrada_publicada_detalle);
                ...
                FAB_edit.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        Toast.makeText(MascotaEncontradaPublicadaDetalleActivity.this, "Proximamente podrás editar tu mascota", Toast.LENGTH_SHORT).show();
                    }
                });
                ...
                FAB_comment.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        Intent i = new Intent(MascotaEncontradaPublicadaDetalleActivity.this, NewCommentActivity.class);
                        startActivity(i);
                    }
                });
                ...
                ViewPagerAdapter adapter = new ViewPagerMascotaEncontradaPublicadaDetalleAdapter(getSupportFragmentManager(), Titles, NumbOfTabs);
    ...
    private void compartir() {
        ...
        if (ShareDialog.canShow(ShareLinkContent.class)) {
            ShareLinkContent linkContent = new ShareLinkContent.Builder()
                    .setContentTitle("¡Encontré una Mascota!")
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        getMenuInflater().inflate(R.menu.menu_mascota_encontrada_publicada_detalle, menu);
        if (((FoundPet) pet).getState().equals(FoundPetState.PUBLISHED)) {
            menu.findItem(R.id.action_share).setVisible(true);
            menu.findItem(R.id.action_delete).setVisible(true);
            menu.findItem(R.id.action_republish).setVisible(false);
        } else if (((FoundPet) pet).getState().equals(FoundPetState.HIDDEN)) {
            menu.findItem(R.id.action_share).setVisible(false);
            menu.findItem(R.id.action_delete).setVisible(false);
            menu.findItem(R.id.action_republish).setVisible(true);
        } else { // Caso Reservada o Reencontrada, no se puede despublicar o republicar.
            menu.findItem(R.id.action_share).setVisible(false);
            menu.findItem(R.id.action_delete).setVisible(false);
            menu.findItem(R.id.action_republish).setVisible(false);
        }
    ...
    private void despublicar() {
        ...
        builder.setMessage("¿Realmente quiere despublicar esta mascota?")
                .setCancelable(false)
                .setPositiveButton("Sí", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int id) {
                        ((FoundPet) pet).setState(FoundPetState.HIDDEN);
                        PetService petService = PetServiceFactory.getInstance();
                        PushService pushService = PushService.getInstance();
                        petService.saveFoundPet((FoundPet) pet);

                        RequestService requestService = RequestService.getInstance();
                        List<FoundRequest> requests = requestService.getAllFoundRequests((FoundPet) pet);
                        for (FoundRequest request : requests) {
                            if (request.getState().equals(RequestState.PENDING)) {
                                request.ignore();
                                requestService.save(request);
                                pushService.sendUnpublishRequestFoundPet(request);
                            }
                        }
                        onBackPressed();
                    }
    ...
    private void republicar() {
        AlertDialog.Builder builder = new AlertDialog.Builder(this);
        builder.setMessage("¿Realmente quiere republicar esta mascota?")
                .setCancelable(false)
                .setPositiveButton("Sí", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int id) {
                        ((FoundPet) pet).setState(FoundPetState.PUBLISHED);
                        PetService petService = PetServiceFactory.getInstance();
                        PushService pushService = PushService.getInstance();
                        petService.saveFoundPet((FoundPet) pet);
                        RequestService requestService = RequestService.getInstance();
                        ...
                        for (FoundRequest request : requests) {
                            if (request.getState().equals(RequestState.IGNORED)) {
                                request.pend();
                                requestService.save(request);
                                pushService.sendRepublishRequestFoundPet(request);
                            }
                        }
                        onBackPressed();
                    }
